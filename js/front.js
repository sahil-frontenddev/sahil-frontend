$(function () {
    ...
    $('.showPallet').on('click',function(e){
        e.preventDefault();
        if ($(this).siblings('.color-palet').hasClass('d-block')){

            $(this).siblings('.color-palet').removeClass('d-block');

        }
        else{
            $(this).siblings('.color-palet').addClass('d-block');
        }
...
function map() {
    ...
    var mapId = 'map',
        mapCenter = [32.2694342,75.6343459],
        mapMarker = true;
    ...
    if ($('#' + mapId).length > 0) {
        ...
        if (mapMarker) {
            ...
            marker.bindPopup("<div class='p-4'><h5>My Home Town Pathankot</h5><p>This is the city where I live.</p></div>", {
                minwidth: 200,
                maxWidth: 600,
...
e.preventDefault();
    ...
    let ele=$('#theme');
    ele.attr('href','css/style.' + theme +'.css');
    $('.color-palet').removeClass('d-block');
}